...
if (isset($_POST['signup'])) {
    ...
    if (count($errors) === 0) {
        ...
        $insert_data = "INSERT INTO user (user_name, user_email, user_phone, user_password,verification_code,user_status) 
...
if (isset($_POST['check'])) {
    ...
    $check_code = "SELECT * FROM user WHERE verification_code = $otp_code";
    ...
    if (mysqli_num_rows($code_res) > 0) {
        ...
        $code = 0;
        $status = 'verified';
        $update_otp = "UPDATE user SET verification_code = $code, user_status = '$status' WHERE verification_code = $fetch_code";
...
if (isset($_POST['check-reset-otp'])) {
    ...
    if (mysqli_num_rows($code_res) > 0) {
        ...
        $_SESSION['info'] = $info;
        header('location: ../user_signup_login/new-password.php');
        exit();
    } else {
        $errors['otp-error'] = "You've entered incorrect code!";
    }
...
if (isset($_POST['change-password'])) {
    ...
    if ($password !== $cpassword) {
        ...
        $encpass = password_hash($password, PASSWORD_BCRYPT);
        $update_pass = "UPDATE user SET verification_code = $code, user_password = '$encpass' WHERE user_email = '$email'";
        $run_query = mysqli_query($con, $update_pass);
        if ($run_query) {
            $info = "Your password changed. Now you can log in with your new password.";
            $_SESSION['info'] = $info;
            header('Location: ../user_signup_login/password-changed.php');
        } else {
