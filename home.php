  ...
  <section class="bg-gradient-to-bl from-blue-100 via-white via-blue-100 to-slate-300 ">
   ...
   <img src="circumference.png" class="h-12  absolute top-[205px] left-[470px]"> -->
      ...
      <div?>
        ...
        <ul id="suggestions" class="suggestions absolute left-[730px] z-10 rounded-bl-lg rounded-br-lg bg-white shadow-lg w-[450px]   mx-auto overflow-hidden cursor-pointer"></ul>
          ...
          searchBox.addEventListener('input', () => {
            ...
            if (query.length > 0) {
              ...
              suggestions.forEach((suggestion, index) => {
                // Create the list item for the suggestion
                const li = document.createElement('li');
                li.textContent = suggestion;
                li.className = 'px-4 py-2 text-lg cursor-pointer hover:bg-blue-100 text-gray-700';
                ...
                // Add a horizontal line after each suggestion except the last one
                if (index < suggestions.length - 1) {
                  const hr = document.createElement('hr');
                  hr.className = 'border-gray-300'; // Tailwind class to style the horizontal line
                  suggestionsBox.appendChild(hr);
                }
              });
            }
        ...
        if ($services_result->num_rows > 0) {
          while ($service = $services_result->fetch_assoc()) {
            ...
            $service_image = 'photo/item/' . $service['service_name'] . '.jpeg';
        ...
        <section class="bg-gradient-to-bl via-white to-blue-50">
          ...
          <!--  Rakib nedd to edit for image -->
          <section class="mt-12  w-[1200px] ml-[450px]">


            <?php
            $sql = "SELECT i.item_id, i.item_name, i.item_details, i.item_picture
        FROM item i
        JOIN (
            ...
            <script>
              // Function to redirect to the item detail page
              function goToDetailsPage(itemName) {
                // Redirect to item details page (assuming the item details page accepts a query parameter 'name')
                window.location.href = `service_details.php?name=${encodeURIComponent(itemName)}`;
              }
            </script>
          </section>
            ...
            <div class="mt-12">
              ...
              <div class="relative">
                ...
                <div id="comments-container" class="flex gap-4 overflow-x-auto scroll-smooth snap-x snap-mandatory overflow-y-hidden">
                  <?php
                  // SQL query to fetch reviews and user details
                  $sql = "SELECT r.*, u.user_name, u.user_profile_picture 
              FROM review_platform r 
              JOIN user u ON r.user_id = u.user_id 
              WHERE r.`user/provider` = 0";

                  $result = $con->query($sql);

                  if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                      $profile_picture = !empty($row['user_profile_picture'])
                        ? './photo/profile_pictures/' . htmlspecialchars($row['user_profile_picture'])
                        : './photo/profile_pictures/default.png';

                      echo '<div class="min-w-[300px] max-w-sm p-4 bg-white rounded-lg shadow-md snap-start border-2 border-blue-200">';
                      echo '<div class="mt-4 flex items-center">';
                      echo '<img src="' . $profile_picture . '" alt="Profile Picture" class="w-20 h-20 rounded-full border border-gray-300">';
                      echo '<div class="ml-3">';
                      echo '<p class="font-semibold text-gray-800">' . htmlspecialchars($row['user_name']) . '</p>';
                      echo '<p class="text-sm text-gray-500 m-12">Rating: ' . htmlspecialchars($row['review_rating']) . '</p>';
                      echo '</div></div>';
                      echo '<p class="text-gray-600 mt-2">"' . htmlspecialchars($row['review_text']) . '"</p>';
                      echo '</div>';
                    }
                  } else {
